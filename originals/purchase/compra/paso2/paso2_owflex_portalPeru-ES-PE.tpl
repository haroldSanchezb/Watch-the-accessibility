		...
		if ({NUM_SEGMENTOS} == 2)
			{
	 		if (!precio_final || precio_final == 0)
				...
				if(!muestra_msg_fee || muestra_msg_fee == 0)
					{					
		 			$('msg-op-id-js').innerHTML = '{_MSG_PRECIOS_OW_JS}';
					}
				else
					{
					$('msg-op-id-js').innerHTML = '{_MSG_PRECIOS_OW_CON_FEE_JS}';
					}
	 			}
	 		else
	 			{
	 			$('msg-op-id-js').innerHTML = '{_MSG_PRECIOS_OW_PRECIO_FINAL_JS}';
	 			}
			}
		}
	}
...
var urlPaso3='{URL_PREFIX}/cgi-bin/{APPLICATION}/paso3.cgi?just_refresh=1;session_id={SESSION_ID}{IS_RESERVA}';
var nroSegmentos = {NUM_SEGMENTOS};
...
function muestra_owflex(fecha_ida, fecha_vuelta)
	{
...
{_BAJADA_PASO_2_OW}	
  ...
  <tr>
  <!-- COMENTADO PORQUE YA NO PONEMOS NOTAS
  {*NOTA}
    <td class="nota"><p> <strong>{_LABEL_NOTA}</strong> <br />
					{_PASO2_FLEX_NOTA} <br />
					{TXT_NOTA} </p></td>
  {/NOTA}
  -->
...
<div id="contenido_tarifas_promocionales_owflex" style="display:none">
 ...
 <br>	
		<div id="msg-op-id-js"></div>
       <p>{MSG_TARIFA_BASE_PE_SOLO_PE} </p>
<!--       {MSG_TUUA_PE} 
       {MSG_ROUNDTRIP_PE}
    <div id="msg_mas_de_un_aeropuerto">{MAS_DE_UN_AEROPUERTO}</div>
{MSG_OPERADORES_ONE_WORLD}
-->
<br> 
 
{*COBRO_PREMIO_OFFLINE_INICIO}
    <ul id="cobro_premio_offline">
        <li class="{CLASS_NO_OFFLINE}" id="tab_no_offline"><a title="{_LABEL_VUELOS_NO_OFFLINE}" onclick="cobro_premio_offline_click(0);"><b>{_LABEL_VUELOS_NO_OFFLINE}</b></a></li>
        <li class="{CLASS_OFFLINE}" id="tab_offline"><a title="{_LABEL_VUELOS_OFFLINE}" onclick="cobro_premio_offline_click(1);"><b>{_LABEL_VUELOS_OFFLINE}</b></a></li>
     </ul>
<div id="box-content">
...
{*BLOQUE_FORMULARIO_OWFLEX_FIN}
	<table cellspacing="0" cellpadding="0" width="40%" id=barra-continuar>
		...
		<tr>
		  ...
		  <td><input type='button' style="float:right;" class="button-process" id='submitButton_owflex' value='{_LABEL_CONTINUAR}' name="submitButton" onclick="check_and_submit_owflex_paso2(document.forms.paso2,urlPaso3,nroSegmentos)"/></td>
		</tr>
	</table>
</form>
{/BLOQUE_FORMULARIO_OWFLEX_FIN}
...
// To allow temporary deactivation of buttons
	if (document.paso2.submitButton)
		...
		advierte_errores_validacion_posteriores(document.paso2);
	        $j(document).ready(function(){onload_owflex()});
</script>
