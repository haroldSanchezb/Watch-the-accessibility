...
<div id="contenido_tarifas_promocionales_owflex" style="display:none">
  ...
  <br>
   {_MSG_FARES_ADULT_DOT}
   <div id="msg_mas_de_un_aeropuerto">{MAS_DE_UN_AEROPUERTO}</div>
<br>
{*COBRO_PREMIO_OFFLINE_INICIO}
    <ul id="cobro_premio_offline">
        <li class="{CLASS_NO_OFFLINE}" id="tab_no_offline"><a title="{_LABEL_VUELOS_NO_OFFLINE}" onclick="cobro_premio_offline_click(0);"><strong>{_LABEL_VUELOS_NO_OFFLINE}</strong></a></li>
        <li class="{CLASS_OFFLINE}" id="tab_offline"><a title="{_LABEL_VUELOS_OFFLINE}" onclick="cobro_premio_offline_click(1);"><strong>{_LABEL_VUELOS_OFFLINE}</strong></a></li>
     </ul>
...
{*BLOQUE_FORMULARIO_OWFLEX_FIN}
  <table width="100%" cellspacing="0" cellpadding="0" id="barra-continuar2" style="float:left;">
    <tr>
    ...
    <tr>
      ...
      <td style="vertical-align:middle;"><table cellspacing="0" cellpadding="0" id=barra-continuar>
          <tr>
            
            <td><input type='button' class="button-process" id='submitButton_owflex' value='{_LABEL_CONTINUAR}' name="submitButton" onclick="check_and_submit_owflex(document.forms.paso2)"/>
            </td>
          </tr>
        </table></td>
    </tr>
