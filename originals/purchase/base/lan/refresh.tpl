...
function chequeo () 
	...
	if (retries == 0)
		{
		location.href = 'wait.cgi?host_id={HOST_ID}&t=0&unique_id={UNIQUE_ID}&check={CHECK}';
		}
	if (im.width == '2')
		{
		location.replace('wait.cgi?host_id={HOST_ID}&show=page&t={TIMEOUT}&unique_id={UNIQUE_ID}&check={CHECK}');
		}
	im = new Image();
	im.src = "wait.cgi?host_id={HOST_ID}&show=image&t="+retries+"&unique_id={UNIQUE_ID}&check={CHECK}";
	setTimeout('chequeo()', {WAIT_TIME}000);
	retries--;
	}
im.src = "wait.cgi?host_id={HOST_ID}&show=image&t={TIMEOUT}&unique_id={UNIQUE_ID}&check={CHECK}";
setTimeout('chequeo()', {WAIT_TIME}000);
// End -->
</script>
</head>
