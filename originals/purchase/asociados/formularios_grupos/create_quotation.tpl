...
<script type="text/javascript">
	...
	function check_and_submit(form)
		...
		var primer_origen = origenes[0].value;
		var ultimo_destino;
		...
		for (var n = 0; n < origenes.length; n++)
			...
			// Minimo dos segmentos
			if ((n < 2) && (!origen || !destino))
				{
				errores += 'Debe ingresar el segmento ' + (n + 1) + "\n";
				}
			...
			// Destino = origen del siguiente
			if (n > 0 && origen)
				{
				if (destinos[n - 1].value && (destinos[n - 1].value != origen))
					{
					errores += 'Origen de segmento ' + (n + 1) + ' debe ser igual a destino de segmento ' + n + "\n";
					}
				}
		...
		// Primer origen = ultimo destino
		if (primer_origen != ultimo_destino)
			{
			errores += 'Primer origen debe ser igual a ultimo destino' + "\n";
			}
...
<form id="formulario" method="POST" action='/cgi-bin/asociados/formularios_grupos.cgi' onSubmit="return false;">

<input type='hidden' name='action'>
<input type='hidden' name='form_name' value='create_quotation'>
...
<p><input type="submit" name="Submit" value="Enviar" class="boton-destacado flotar_der" onClick="check_and_submit(document.getElementById('formulario'));"></p>
