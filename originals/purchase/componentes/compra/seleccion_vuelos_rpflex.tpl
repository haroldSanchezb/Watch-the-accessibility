...
{*BLOQUE_SERVICE_FEE}
<script type="text/javascript">
	jQuery.data(document.body, 'feeChargeData', {DATOS_COBRO_FEE});
	var feeOverFareQ = {FEE_CON_FARE_Q} || 0;
</script>
{/BLOQUE_SERVICE_FEE}
...
function calcula_tarifa ()
	...
	if (tarifa != '' || (typeof(tarifa) == 'number' && tarifa == 0))
		...
		if ('{FLAVOUR}' == 'cplp_variable' || '{FLAVOUR}' == 'cambio_fecha_cplp_variable')
		...
		else if (precio_final == 1 || is_rpflex == 1)
			{
			...
			$('tarifa_sin_impuesto').innerHTML = '{SIGNO_MONEDA}' + precio_total + '<br>{_LABEL_POR_PASAJERO}';
			
			$('taxes_final_paso2').value = total_impuestos;
			total_impuestos = currency_format(total_impuestos, '{ID_MONEDA}');
			$('total_impuestos').innerHTML = '{SIGNO_MONEDA}' + total_impuestos + '<br>{_LABEL_POR_PASAJERO}';
			...
			if (is_rpflex == 1) {
				...
				$('total_q').innerHTML = '{SIGNO_MONEDA}' + currency_format(total_q, '{ID_MONEDA}') + '<br>{_LABEL_POR_PASAJERO}';
				$('total_fee').innerHTML = '{SIGNO_MONEDA}' + currency_format(total_fee, '{ID_MONEDA}') + '<br>{_LABEL_POR_PASAJERO}';
			...
			$('sumatarifas').innerHTML = '{SIGNO_MONEDA}' + precio_final + '<br><span style="font-size:11px;">{_LABEL_POR_PASAJERO}</span>';
			...
			$('sumatarifas').innerHTML = '{SIGNO_MONEDA}' + precio_total + '<br><span style="font-size:11px;">{_LABEL_POR_PASAJERO}</span>';
			}
...
function get_total_impuestos(tarifa)
	...
	var tarifa_ida = tarifa_vuelta = tarifa;
	var d;
	var impuestos_cobrados = new Hash({});
		...
		for (var i = 0;i<impuestos_ida.length;i++)
			...
			h.each(function(p) {
				...
				if (d['monto'] > 0)
					...
					{
					if (!impuesto_aplicado(p.key, impuestos_cobrados))
						{
...
function calcula_monto_especial(tax, porcentaje, aplica_sobre, tarifa, impuestos_ref)
	...
	if (match_fare != null)
		{
		result += currency_round_tax(tarifa * porcentaje/100, '{ID_MONEDA}', tax);
...
<div id="id_cont_ow" class="cont_ow">
{LAYER_ESCALAS}
...
<!--				<p style="font-size: 11px; font-weight:normal; margin-bottom: 0;">
  ...
  <tr>
    <td colspan=4>{*FLECHA_AYER}<a href='{LINK_PASO2_AYER}' class="sinlinea"><img src='/images/espanol/botones/bullet_ant.gif' alt='{_LABEL_DIA_ANTERIOR}' width="7" height="8" border='0'></a>&nbsp;{/FLECHA_AYER} {FECHA_VUELO}{*FLECHA_MANANA}&nbsp;<a href='{LINK_PASO2_MANANA}' class="sinlinea"><img src='/images/espanol/botones/bullet_sgte.gif' alt='{_LABEL_DIA_SIGUIENTE}' width="7" height="8" border='0'></a>{/FLECHA_MANANA}</td>
		{*FAMILIASOW}
		<td rowspan="2" style="width: 62px; background-color: {FF_BG_COLOR}; color: {FF_FG_COLOR}; vertical-align: top !important; text-align: center; padding: 2px 4px; border-left: solid 1px white;" onMouseOver="if (typeof(activa_ttip) != 'undefined') activa_ttip('ttip_familia_{ID_TTIP}', 'tooltip_familia_{ID_FAMILIA}');" onMouseOut="if (typeof(desactiva_ttip) != 'undefined') desactiva_ttip();">
			<div style="font-size: 13px; display: block; height: 2.5em; font-weight: bold;">{FARE_FAMILY}</div>
			{FARE_FAMILY_DESC}
  ...
  <tr>
    <th style="width:150px;">{_LABEL_VUELOS}</th>
<!--    <th><a href='{SCRIPT_URL}?session_id={SESSION_ID}&amp;ordenar_vuelos_por=departure_time&amp;just_refresh=1{OWFLEX_ONEWORLD}'>{_LABEL_SALIDA}</a></th>
 ...
 {*MATERIAL_DE_VUELO}<img src='/images/html/avion1.gif' alt='{_LABEL_AVION}' width="12" height="12" border='0'> {AVION}{/MATERIAL_DE_VUELO}<br/>{ESCALAS}{/INFO_TRANSBORDO}</td>
		<td style="border-top: solid 1px #f0f0f0; padding: 8px 4px 4px 4px;">
		...
		<td style="border-top: solid 1px #f0f0f0; padding: 8px 4px 4px 4px;"><span style="vertical-align:top">{DURACION}&nbsp;hrs.</span></td>
	...
	<td id="tdfamilia{ID_FAMILIA}[{ID_SEGMENTO}][{ID_VUELO}]" style="background-color: {FF_BG_COLOR}; color: {FF_FG_COLOR}; border-top: solid 1px white; border-left: solid 1px white; font-weight: bold; cursor: pointer; text-align: center" onClick="($('radiofamilia{ID_FAMILIA}[{ID_SEGMENTO}][{ID_VUELO}]')).checked = true;set_vuelo_y_clase($('radiofamilia{ID_FAMILIA}[{ID_SEGMENTO}][{ID_VUELO}]').form, '{ID_SEGMENTO}', '{ID_VUELO}', '{CLASE}', '{ID_FAMILIA}', '{MONEDA}', '{VALOR_TARIFA}','{FAREBASIS_FULL}','{NAME}', {ARRAY_TAXES}); javascript:redirect_paso2_manana('{RELOAD}', '{LINK_PASO2_MANANA_VUELTA}', 'radiofamilia{ID_FAMILIA}[{ID_SEGMENTO}][{ID_VUELO}]', '0', '{OVERNIGHT}');" onMouseOver="if (typeof(activa_ttip) != 'undefined') activa_ttip('ttip_{ID_FAMILIA}[{ID_SEGMENTO}][{ID_VUELO}]', 'tooltip_tarifa');" onMouseOut="if (typeof(desactiva_ttip) != 'undefined') desactiva_ttip();">
		...
		<input type="hidden" id="fee_tarifa_seg{ID_SEGMENTO}_vue{ID_VUELO}_fam{ID_FAMILIA}" value="{FEE}"><br>
...
<div id="id_cont_it_pf" class="cont_it_pf">
...
<h4 style="font-size: 12px;">{_LABEL_TARIFA}</h4>
...
<table cellspacing="0" width="100%" class="table_process" style=" background-color: #FFFFFF;">
<tr style="font-size: 9pt">
	<td style="text-align:left; padding:4px 4px 4px 8px;">{LABEL_TARIFA}:</td>
	<td id = "tarifa_sin_impuesto" style="text-align:right; padding:8px 8px 4px 8px;">{_LABEL_VALOR_PAGAR_SIN_IMPUESTOS}</td>
</tr>
<tr style="font-size: 9pt">
	<td style="text-align:left; padding:4px 4px 4px 8px;">{_LABEL_FEE_CO}:</td>
	<td id ="total_fee" style="text-align:right; padding:8px 8px 4px 8px;">{_LABEL_TOTAL} {_LABEL_FEE}</td>
</tr>
<tr style="font-size: 9pt">
	<td style="text-align:left; padding:4px 4px 4px 8px;">{_LABEL_SURCHARGES}:</td>
	<td id = "total_q" style="text-align:right; padding:8px 8px 4px 8px;">{_LABEL_TOTAL} {_LABEL_SURCHARGES}</td>
</tr>
<tr style="font-size: 9pt">
	<td style="text-align:left; padding:4px 4px 10px 8px;">{_LABEL_IMPUESTOS}:</td>
	<td id ="total_impuestos" style="text-align:right; padding:8px 8px 10px 8px;">{_LABEL_TOTAL} {_LABEL_IMPUESTOS}</td>
</tr>
<tr style="font-size: 10pt">
	<td style="text-align:left; color:#333333; padding:10px 4px 4px 8px; border-top:1px solid #ccc; color:#003E7E; vertical-align:top;"><b>{LABEL_TARIFA} (*) : &nbsp;</b></td>
	<td id="sumatarifas" style="font-weight: bold; text-align:right; color:#333333; padding:10px 8px 4px 8px; border-top:1px solid #ccc; color:#003E7E; font-size:18px;">&nbsp;</td>
</tr>
<tr style="font-size: 9pt">
	<input type='hidden' name='moneda_precio_final_paso2' id='moneda_precio_final_paso2' value='{MONEDA_FINAL_PASO2}'>
	<td colspan="2" style=" font-size:11px; color:#333333; text-align:center;">(*) {_VALORES_EN} {MONEDA}</td>
</tr>
</table>
{/BLOQUE_TARIFA_CON_TAX}
{/BLOQUE_DESPLIEGUE_TARIFA}
...
var initial_detalle_tarifa_offsettop = document.getElementById('id_cont_it_pf').offsetTop;
	
	Event.observe(window, 'scroll', function() {
		var posy;
		var offset_from_top = 10;
		if(getScrollY() > initial_detalle_tarifa_offsettop){
			posy = getScrollY() - initial_detalle_tarifa_offsettop + offset_from_top;
		}else {
			posy = 0;
		}
		var owflex_height = document.getElementById('id_cont_ow').offsetHeight;
		var detalle_tarifa_height = document.getElementById('id_cont_it_pf').offsetHeight;
		if (posy < (owflex_height - detalle_tarifa_height) ) {
			$('id_cont_it_pf').setStyle({marginTop: posy + 'px'});
		}                                                   
