{_MSG_DESPLIEGUE_ROWFLEX}</li>

<input type='hidden' id='fecha_vuelta_min' value='{FECHA_IDA_MIN}'>
<input type='hidden' id='fecha_vuelta_max' value='{FECHA_IDA_MAX}'>
...
{*BLOCK_OW}
	<table width="100%" border="0" cellpadding="0" cellspacing="2" class="flex_matrix">
		<tr>
			...
			{*COLUMNA_IDA_HEADER}
			<th class="unselected" id="vuelta_{FECHA}"> 
				  <!-- {_LABEL_SALIENDO}-->
				<b>{FECHA_FORMAT}</b>
		  </th>
		...
		<tr>
			...
			{*COLUMNA_IDA}
			<td class="color_{COLOR} unselected" id="vuelta_fechas_{FECHA_IDA}|{FECHA_VUELTA}" onMouseOver="set_way(this);celda_mouse_over({FORM_NAME}, '{FECHA_IDA}', '');" onMouseOut="set_way(this);celda_mouse_out({FORM_NAME}, '{FECHA_IDA}', '{FECHA_VUELTA}', 'color_{COLOR}');" onClick="set_way(this);celda_mouse_click({FORM_NAME}, '{FECHA_IDA}', '{FECHA_VUELTA}'); {DESPLIEGA_OWFLEX}" style='text-align: center; vertical-align: middle; border:1px solid #EAEAEA; padding-top:2px; padding-bottom: 2px;'>
			<input type="hidden" id="vuelta_fechas_{FECHA_IDA}|{FECHA_VUELTA}_color" value="color_{COLOR}"/>
			<input type="hidden" id="vuelta_fechas_{FECHA_IDA}|{FECHA_VUELTA}_bg_familia" value="{BG_COLOR_FAMILIA}"/>
			<input type="hidden" id="vuelta_fechas_{FECHA_IDA}|{FECHA_VUELTA}_fg_familia" value="{FG_COLOR_FAMILIA}"/>

			<input name="fechas" type="radio" id="vuelta_radio_{FECHA_IDA}|{FECHA_VUELTA}" value="{FECHA_IDA}|{FECHA_VUELTA}" style='display:none' onclick="set_way(this);set_fechas(this.form, '{FECHA_IDA}', '{FECHA_VUELTA}');"/>
			{SIGNO}{TARIFA}
			</td>
			{/COLUMNA_IDA} 
...
function init_vuelta() {
	window.onLoad = init({FORM_NAME}, '{FECHA_IDA_PRESEL}', 'vuelta');
}
</script>
