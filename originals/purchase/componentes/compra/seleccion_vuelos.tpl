				...
				if (radio)
					...
					if (radio.checked)
						...
						if (precio_final)
							...
							var q_tarifa_seg_vue_fam = $('q_tarifa_seg' + i + '_vue' + j + '_fam' + family) ;
							if (q_tarifa_seg_vue_fam && q_tarifa_seg_vue_fam.value) 
								{
								total_q += parseFloat(q_tarifa_seg_vue_fam.value);
								}
						}
					}
				}
	...
	if (tarifa != '' || (typeof(tarifa) == 'number' && tarifa == 0))
		...
		*/
		if ('{FLAVOUR}' == 'cplp_variable' || '{FLAVOUR}' == 'cambio_fecha_cplp_variable')
			...
			var home = '{HOME}';
			var tarifa_administrativa = '{SOBRECARGO}';
			...
			$('tarifa_administrativa_final_paso2').value = tarifa_administrativa;
			if (home == 'es/co') {
				precio_final = precio_final + parseInt(tarifa_administrativa);
				tarifa_administrativa = currency_format(parseFloat(tarifa_administrativa),'{ID_MONEDA}');
				$('tarifa_administrativa').innerHTML = '{SIGNO_MONEDA}' + tarifa_administrativa ;
			}
...
function activa_flight_info(operado_por, marketeado_por, id_class, flight_stats_query) {
	...
	var obj = jQuery("#following_tooltip");
	obj[0].innerHTML = document.getElementById("ttip_flight_info").innerHTML;
	...
	obj[0].className = id_class;
	document.onmousemove = follow;
	
}
function desactiva_flight_info() {
	jQuery("#following_tooltip").css("visibility", "hidden");
...
<input type='hidden' name='tarifa_administrativa_final_paso2' id='tarifa_administrativa_final_paso2' value = ''>
...
<!--				<p style="font-size: 11px; font-weight:normal; margin-bottom: 0;">
  ...
  <tr>
    <td colspan=4>{*FLECHA_AYER}<a title="{_LABEL_DIA_ANTERIOR}" href='{LINK_PASO2_AYER}' class="sinlinea"><img src='/images/espanol/botones/bullet_ant.gif' alt='{_LABEL_DIA_ANTERIOR}' width="7" height="8" border='0'></a>&nbsp;{/FLECHA_AYER} {FECHA_VUELO}{*FLECHA_MANANA}&nbsp;<a title="{_LABEL_DIA_SIGUIENTE}" href='{LINK_PASO2_MANANA}' class="sinlinea"><img src='/images/espanol/botones/bullet_sgte.gif' alt='{_LABEL_DIA_SIGUIENTE}' width="7" height="8" border='0'></a>{/FLECHA_MANANA}</td>
		{*FAMILIASOW}
...
<tr style="font-size: 12px">
    <td class="par" style="text-align:left;">{_LABEL_TARIFA_ADMINISTRATIVA}</td>
	<td id ="tarifa_administrativa" class="par" style="text-align:right;">{_LABEL_TARIFA_ADMINISTRATIVA}</td>
</tr>
...
<tr style="font-size: 12px">
<input type='hidden' name='moneda_precio_final_paso2' id='moneda_precio_final_paso2' value='{MONEDA_FINAL_PASO2}'>
<input type='hidden' name='sobrecargo' id='sobrecargo' value='{SOBRECARGO}'>
