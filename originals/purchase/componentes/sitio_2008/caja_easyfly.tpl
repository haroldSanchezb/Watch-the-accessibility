<script type="text/javascript">
	...
	function vuelos_set_fecha_from_text(id, text)
		{
		var pattern_format = /([0-9]{2})\/([A-Z]{3})\/([0-9]{4})/;
		var res_other_pattern = pattern_format.exec(text);
		var array_text;
		if (res_other_pattern)
			{
			array_text = text.split("/");
			numero_mes = get_mes_numero(array_text[1]);
			text = array_text[0] + "/" + numero_mes + "/" + array_text[2];
			}
		var pattern = /([0-9]{2})\/([0-9]{2})\/([0-9]{4})/;
	...
	function vuelos_check_and_submit(f)
		{
		var errores, i, elem, elem2, fecha_segmento1, fecha_segmento2, from, to;

		errores = '';
		from = to = '';
		f = document.forms['paso1_easyfly'];
		// Chequeo de origen - destino
		elem = f.from_city1;
		if (elem && Empty(elem.options[elem.selectedIndex].value))
			{
		...
		elem = f.fecha2_dia;
		elem2 = f.fecha2_anomes;
		if (getRadioValue(f.ida_vuelta) == 'ida_vuelta')
			{
			if (!elem.value || !elem2.value)
				{
	...
	function vuelos_completa_campos_hidden(f)
		...
		if (getRadioValue(f.ida_vuelta) == 'ida_vuelta')
			{
			x = f.to_city1;
			f.from_city2.value = x.options[x.selectedIndex].value;
			x = f.from_city1;
			f.to_city2.value = x.options[x.selectedIndex].value;
			f.num_segmentos_interfaz.value = '2';
			}
		else
			{
			f.from_city2.value = '';
			f.to_city2.value = '';
			f.num_segmentos_interfaz.value = '1';
			}
		}
...
<form name="paso1_easyfly" action="/no_js.html" method="GET">
	{*BLOQUE_NO_LOGUEADO}
	<div id="primer_bloque">
		{_MSG_CAJA_COMPRA_EASYFLY_NO_TRAMOS}
	</div>
	<div id="bloque_mensaje_no_logueado">
	</div>	
	<div id="footer_navegacion">
		<table>
			<tr>
			<td class="ancho"><a href="/cgi-bin/compra_cupon_easyfly/paso1.cgi">{_MSG_COMPRAR_CUPONERA}</a></td>
                        <td class="ancho"><a href="/cgi-bin/empresas/easyfly/consulta_cupones.cgi">{_LABEL_REPORTE_CUPONES}</a></td>
                        <td class="ancho"><a href="/cgi-bin/empresas/easyfly/elimina_titulares.cgi">{_LABEL_ELIMINAR_TITULARES}</a></td>
			</tr>
		</table>
	</div>
	...
	<input type="hidden" name="canje_cupon_caja_compra" value="1">
	<input type="hidden" name="accion" value="buscar">

	<div id="primer_bloque">
		<div class="bloque_mensaje_easyfly">{_MSG_CAJA_COMPRA_EASYFLY}</div>
	</div>
	...
	<div id="bloque_direccion">
		<div class="bloque_ida_vuelta">
			<p class="radios">
				<input type="radio" name="ida_vuelta" class="radio" value="ida_vuelta" onClick="vuelos_set_ida_y_vuelta(this.value);" checked><span class="label_radio">{_LABEL_IDA_Y_VUELTA}</span>
			</p>
		</div>

		<div class="bloque_solo_ida">
				
			<p class="radios"><input type="radio" name="ida_vuelta" class="radio" value="solo_ida" onClick="vuelos_set_ida_y_vuelta(this.value);"><span class="label_radio">{_LABEL_SOLO_IDA}</span>
			</p>
		</div>
		<div class="bloque_ayuda" style="text-align: right;">

			<p></p>
		</div>
		</div>
		<div id="bloque_ciudad">		
		<div class="bloque1_2">
	...
	<div id="tercer_bloque">
	<div class="bloque_fecha_salida">
	<label for="vuelos_fecha_salida">{_LABEL_FECHA_IDA}</label>
			<input type="text" class="text_easfly fecha" name="vuelos_fecha_salida" id="vuelos_fecha_salida" value="" onChange="vuelos_set_fecha_from_text(1, this.value);" size="11"><img src="/images/common/calendario.jpg" id="cal1" class="calendario" alt="" width="25" height="20">
			<input type="hidden" class="text_easfly fecha" name="vuelos_fecha_salida_ddmmaaaa" id="vuelos_fecha_salida_ddmmaaaa" onChange="vuelos_set_fecha_from_text(1, this.value);">
			<script type="text/javascript">
				Calendar.setup({
			...
			<div class="bloque_fecha_regreso" id = "vuelos_panel_fecha_regreso">
			<label for="vuelos_fecha_regreso">{_LABEL_FECHA_REGRESO}</label>
			<input type="text" class="text_easfly fecha" name="vuelos_fecha_regreso" id="vuelos_fecha_regreso" value="" onChange="vuelos_set_fecha_from_text(2, this.value);" size="11">
			<input type="hidden" class="text_easfly fecha" name="vuelos_fecha_regreso_ddmmaaaa" id="vuelos_fecha_regreso_ddmmaaaa" value="" onChange="vuelos_set_fecha_from_text(2, this.value);" >
	...
	<div id="cuarto_bloque">
		<div id="bloque_cabina">
			<label for="vuelos_cabina">{_LABEL_CABINA}</label><br />

			<select name="cabina" id="vuelos_cabina">
				{CABINAS}
			</select>
		</div>
		<div id="bloque_boton_easyfly">
		<div class="boton_continuar_easyfly_rojo">
			<img src="/images/common/borde_izq_boton_rojo.jpg" alt="" width="6" height="20">
			<a href="javascript:vuelos_check_and_submit(this);" class="btn_rojo_easyfly">{_LABEL_CONTINUAR} &raquo;</a> 
			<img src="/images/common/borde_der_boton_rojo.jpg" alt="" width="6" height="20">
			</div>
		</div>	
	</div>
	<div id="quinto_bloque">
		<div id="bloque_links_cupones_activos">
		<a href="/cgi-bin/empresas/easyfly/consulta_cupones.cgi?accion=consultar&amp;estado_cupon=NO_CANJEADO">{_LABEL_VER_CUPONES_ACTIVOS}</a>		</div>
